...
describe('repos', () => {
  ...
  test('whereByProfileName', async () => {
    ...
    expect(await dr.user.whereByProfileName('ali').getOptional('id')).toBe(u1.id);
    expect(await dr.user.whereByProfileName('bob').getOptional('id')).toBe(u2.id);
    expect(await dr.user.whereByProfileName('').getOptional('id')).toBe(u3.id);
    expect(await dr.user.whereByProfileName(' ').getOptional('id')).toBe(u4.id);
    expect(await dr.user.whereByProfileName(undefined!).getOptional('id')).toBeUndefined();
    expect(await dr.user.whereByProfileName(null!).getOptional('id')).toBeUndefined();
  });
});
